...
SCAN_INTERVAL  = 15    # secondi base tra una scansione e l'altra
JITTER_MAX     = 6     # secondi casuali aggiunti ad ogni scansione (antiban)
SESSION_SPINS  = 14
MAX_ERRORS     = 3
FETCH_RETRIES  = 3
...
# ─── ANTIBAN: pool di User-Agent realistici ────────────────────────────────────
...
# ─── FETCH HTML senza proxy, con antiban ──────────────────────────────────────

def fetch_html(url):
    # type: (str) -> Optional[str]

    # Pausa casuale breve prima di ogni richiesta (antiban)
    ...
    for attempt in range(1, FETCH_RETRIES + 1):
        try:
            ...
            logger.warning("[fetch] Timeout tentativo %d su %s", attempt, url)
            time.sleep(4)
        except Exception as e:
...
def extract_tracksino(html):
    ...
    for seg in soup.find_all(class_=re.compile(r"game-stats-seg")):
        ...
        if not m:
            continue
        val = int(m.group(1))
        if re.search(r'Crazy Time 10 Segment', alt, re.IGNORECASE):
            spins_since_10 = val
...
def extract_casinoscores(html):
    ...
    for script in soup.find_all("script"):
        ...
        if "10" in results and "1" in results:
            break

    return results.get("10"), results.get("1")
...
def scrape_all_sources():
    ...
    if html:
        try:
            val10, val1, last_result = extract_tracksino(html)
        except Exception as e:
            logger.warning("[tracksino] Errore estrazione: %s", e)
            val10, val1, last_result = None, None, None
        if val10 is not None:
            logger.info("[tracksino] spins_since_10=%s | spins_since_1=%s", val10, val1)
            state["last_source"] = "tracksino"
            return val10, val1, last_result
        else:
            logger.warning("[tracksino] Nessun dato '10' trovato nell'HTML")
    else:
    ...
    if html:
        try:
            val10, val1 = extract_casinoscores(html)
        except Exception as e:
            logger.warning("[casinoscores] Errore estrazione: %s", e)
            val10, val1 = None, None
        if val10 is not None:
            logger.info("[casinoscores] spins_since_10=%s | spins_since_1=%s", val10, val1)
            state["last_source"] = "casinoscores"
            return val10, val1, None
        else:
            logger.warning("[casinoscores] Nessun dato '10' trovato")
    else:
...
def _detect_appeared_10(spins_since_10, prev_10, spins_since_1, prev_1, in_session):
    if spins_since_1 is not None and prev_1 is not None:
        ...
        logger.info(
            "Differenziale: 10: %s->%s | 1: %s->%s | appeared_10=%s | session=%s",
            prev_10, spins_since_10, prev_1, spins_since_1, appeared_10, in_session
        )
        return appeared_10
    else:
        if spins_since_10 == prev_10:
            if spins_since_10 == 0 and in_session:
                logger.info("Fallback: 10 consecutivo rilevato (prev=0 curr=0 in sessione)")
                return True
            return None
        appeared_10 = (spins_since_10 < prev_10)
        logger.info(
            "Fallback: 10: %s->%s | appeared_10=%s | session=%s",
            prev_10, spins_since_10, appeared_10, in_session
        )
        return appeared_10
...
def bot_loop():
    ...
    while state["running"]:
        ...
        # Pausa base + jitter casuale (antiban)
